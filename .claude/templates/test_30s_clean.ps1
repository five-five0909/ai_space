# run_experiment.ps1
# 30 second test script - Pure ASCII with notification
...
function Invoke-Notification {
    ...
    # Get workdir from status file
    $notifierScript = $null
    try {
        $status = Get-Content "$ExpDir\experiment_status.json" -Encoding UTF8 | ConvertFrom-Json
    ...
    if (-not $notifierScript) {
        Write-Log "WARNING: Notifier not found"
        return
    }

    try {
        & powershell -NoProfile -File $notifierScript -ExperimentDir $ExpDir
        Write-Log "Notification generated"
    } catch {
        Write-Log "WARNING: Notification failed: $_"
    }
}
