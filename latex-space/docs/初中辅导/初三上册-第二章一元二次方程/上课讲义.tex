...
\begin{examplebox}
    \begin{enumerate}[label=\textbf{例\arabic*.}]
        ...
        \item \textbf{（概念陷阱）} 若函数 $y = (m-2)x^{m^2-2} + 3x$ 是关于 $x$ 的二次函数，则 $m$ 的值为 \underline{\hspace{2cm}}。
        
        \item 将二次函数 $y = 2(x-1)(x+3)$ 化为一般式 ($y=ax^2+bx+c$)，其中 $a=$\underline{\hspace{1cm}}, $b=$\underline{\hspace{1cm}}, $c=$\underline{\hspace{1cm}}。
    \end{enumerate}
\end{examplebox}
...
\begin{answerbox}
    \begin{enumerate}
        \item \textbf{答案：D} \\ 
        解析：A是一次函数；B化简后为 $y = -2x + 1$，是一次函数；C分母含有自变量；D符合定义。
        
        \item \textbf{答案：-2} \\ 
        解析：由定义知：指数 $m^2 - 2 = 2$，且系数 $m - 2 \neq 0$。
        解得 $m^2 = 4 \Rightarrow m = \pm 2$。但 $m \neq 2$，所以 $m = -2$。
        
        \item \textbf{答案：2, 4, -6} \\ 
        解析：展开括号：$y = 2(x^2 + 2x - 3) = 2x^2 + 4x - 6$。
    \end{enumerate}
\end{answerbox}
...
\begin{examplebox}
    \begin{enumerate}[label=\textbf{例\arabic*.}]
        ...
        \item 已知点 $A(1, y_1)$， $B(2, y_2)$， $C(-3, y_3)$ 都在抛物线 $y = -x^2$ 上，则 $y_1, y_2, y_3$ 的大小关系是 \underline{\hspace{2cm}}。
    \end{enumerate}
\end{examplebox}
...
\begin{answerbox}
    \begin{enumerate}
        \item \textbf{答案：B} \\
        解析：$y=ax^2$ 形式的抛物线，对称轴永远是 $y$ 轴。
        $A$错(一正一负)；$C$错(正向上无最高点)；$D$错($|3| \neq |-\frac{1}{3}|$，宽度不同)。
        
        \item \textbf{答案：$y_3 < y_2 < y_1$} \\
        解析：\textbf{方法一（代入法）：} $y_1 = -1^2 = -1, y_2 = -2^2 = -4, y_3 = -(-3)^2 = -9$。显然 $-9 < -4 < -1$。\\
        \textbf{方法二（图象法）：} $a=-1<0$，开口向下。离对称轴($y$轴)越远的点，位置越低。$|-3| > |2| > |1|$，故 $C$ 最低，$A$ 最高。
    \end{enumerate}
\end{answerbox}
...
\begin{examplebox}
    \begin{enumerate}[label=\textbf{例\arabic*.}]
        \setcounter{enumi}{5}
        \item 抛物线 $y = -2(x+3)^2 - 5$ 的顶点坐标是 \underline{\hspace{2cm}}，对称轴是直线 \underline{\hspace{2cm}}。
        
        \item 将抛物线 $y = 3x^2$ 向右平移 2 个单位，再向上平移 1 个单位，得到的抛物线解析式为 \underline{\hspace{3cm}}。
        
        \item \textbf{（逆向思维）} 将抛物线 $y = (x-1)^2 + 3$ 向左平移 2 个单位，再向下平移 4 个单位，得到的新抛物线与 $y$ 轴的交点坐标是 \underline{\hspace{2cm}}。
    \end{enumerate}
\end{examplebox}

\begin{answerbox}
    \begin{enumerate}
        \item \textbf{答案：$(-3, -5)$; $x=-3$} \\
        解析：对比 $y=a(x-h)^2+k$。注意括号里是 $x+3$，即 $x-(-3)$，所以 $h=-3$。括号外直接读 $k=-5$。
        
        \item \textbf{答案：$y = 3(x-2)^2 + 1$} \\
        解析：原顶点 $(0,0)$。
        向右移2 $\rightarrow$ $x$变为 $x-2$；向上移1 $\rightarrow$ 尾巴加1。
        
        \item \textbf{答案：$(0, 0)$} \\
        解析：
        第一步：找原顶点 $A(1, 3)$。
        第二步：移动顶点。左移2 ($1-2=-1$)，下移4 ($3-4=-1$)。新顶点 $A'(-1, -1)$。
        第三步：写新解析式。$a$ 不变仍为 1。$y = 1(x - (-1))^2 - 1 = (x+1)^2 - 1$。
        第四步：求交点。令 $x=0$，得 $y = (0+1)^2 - 1 = 1 - 1 = 0$。故交点为 $(0,0)$。
    \end{enumerate}
\end{answerbox}
...
\begin{answerbox}
    ...
    解析：
    \begin{itemize}
        \item A错：开口向下 $\Rightarrow a < 0$。
        \item B错：与 $y$ 轴交点在 $x$ 轴上方 $\Rightarrow c > 0$。
        \item C错：对称轴 $x=1$ 在右侧，"左同右异" $\Rightarrow a,b$ 异号。因 $a<0$，故 $b>0$。
        \item D对：图象与 $x$ 轴有两个交点 $\Rightarrow \Delta = b^2 - 4ac > 0$。
    \end{itemize}
\end{answerbox}
...
\begin{answerbox}
    \begin{enumerate}
        ...
        解析：从图象读取信息：
        \begin{itemize}
            \item 开口向上 $\Rightarrow a > 0$
            \item 对称轴 $x = -\dfrac{1}{2}$ 在左侧，由"左同右异"知 $b > 0$（$a,b$同号）
            \item 与 $y$ 轴交于负半轴 $\Rightarrow c < 0$
        \end{itemize}
        因此：
        \begin{itemize}
            \item $\dfrac{b}{a} > 0$（正数除以正数）
            \item $\dfrac{c}{a} < 0$（负数除以正数）
        \end{itemize}
        点 $M\left(\dfrac{b}{a}, \dfrac{c}{a}\right)$ 横坐标为正，纵坐标为负，在\textbf{第四象限}。
        
        \item \textbf{答案：C} \\
        解析：从图象读取信息：
        \begin{itemize}
            \item 开口向下 $\Rightarrow a < 0$
            \item 对称轴 $x = 1$ $\Rightarrow -\dfrac{b}{2a} = 1$ $\Rightarrow b = -2a > 0$
            \item 与 $y$ 轴交于正半轴 $\Rightarrow c > 0$
            \item 与 $x$ 轴交于 $(-1,0)$ 和 $(2,0)$ 附近
        \end{itemize}
        逐项判断：
        \begin{itemize}
            \item (1) $a+b+c$ 是 $x=1$ 时的函数值，由图 $y(1) > 0$ \checkmark
            \item (2) $4a+2b+c$ 是 $x=2$ 时的函数值，由图 $y(2) = 0$，不大于0 \textbf{$\times$}
            \item (3) 由 $b = -2a$ 得 $2a+b = 0$ \checkmark
            \item (4) 由对称性，$x=-1$ 和 $x=3$ 函数值相等，$y(-1)=0$，则 $y(3)=9a+3b+c=0$，即 $3a+c=-3a-b>0$（因为 $a<0$）\checkmark
        \end{itemize}
        正确的有3个。
        ...
        \begin{itemize}
            \item 开口向上 $\Rightarrow a > 0$
            \item 对称轴 $x = 1$ 在右侧 $\Rightarrow b < 0$（与 $a$ 异号）
            \item 与 $y$ 轴交于负半轴 $\Rightarrow c < 0$
            \item 过点 $(-1,0)$，由对称性另一交点为 $(3,0)$
        \end{itemize}
        逐项判断：
        \begin{itemize}
            \item (1) $a>0, b<0, c<0$，则 $abc > 0$ \textbf{$\times$}
            \item (2) 当 $x=-1$ 时，$a-b+c=0$，即 $b=a+c$。因 $a>0$，故 $b > c$ \textbf{$\times$}
            \item (3) 由对称轴 $-\dfrac{b}{2a}=1$ 得 $b=-2a$。代入 $a-b+c=0$：$a-(-2a)+c=3a+c=0$ \checkmark
            \item (4) $a+b$ 是 $x=1$ 时的函数值（最小值）。$m(am+b) = am^2+bm$ 是 $x=m$ 时的函数值减 $c$。实际上这是说 $x=1$ 时取得最小值，对任意 $m$ 有 $a+b+c \leqslant am^2+bm+c$，即 $a+b \leqslant m(am+b)$ \checkmark
        \end{itemize}
        正确的有2个，答案应为B。详细验证(4)：$x=1$是最小值点，故对任意 $m$，$f(1) \leqslant f(m)$，即 $a+b+c \leqslant am^2+bm+c$，化简得 $a+b \leqslant am^2+bm = m(am+b)$。
    \end{enumerate}
\end{answerbox}
...
\begin{answerbox}
    \begin{enumerate}
        ...
        \begin{enumerate}
            \item[(1)] \textbf{直接开平方法：} \\
            $2x - 1 = \pm 4$ \\
            当 $2x - 1 = 4$ 时，$x_1 = \dfrac{5}{2}$ \\
            当 $2x - 1 = -4$ 时，$x_2 = -\dfrac{3}{2}$
            ...
            $x = \dfrac{4 \pm \sqrt{24}}{4} = \dfrac{4 \pm 2\sqrt{6}}{4} = \dfrac{2 \pm \sqrt{6}}{2}$
        \end{enumerate}
        
        \item \textbf{答案：A} \\
        解析：能用因式分解，说明方程可写成 $(x-a)(x-b)=0$ 的形式。
        展开得 $x^2 - (a+b)x + ab = 0$。
        对比 $x^2 - 4x + m = 0$，需要 $a + b = 4$。
        \begin{itemize}
            \item A. $m=3$：$x^2 - 4x + 3 = (x-1)(x-3) = 0$ \checkmark
            \item B. $m=5$：判别式 $16-20=-4<0$，不能分解
            \item C. $m=6$：$x^2 - 4x + 6$，判别式 $16-24=-8<0$
            \item D. $m=8$：判别式 $16-32=-16<0$
        \end{itemize}
    \end{enumerate}
\end{answerbox}
...
\begin{answerbox}
    \begin{enumerate}
        \item \textbf{解答：}
        \begin{enumerate}
            \item[(1)] $\Delta = (-3)^2 - 4 \times 2 \times 1 = 9 - 8 = 1 > 0$，\textbf{两个不等实根}
            \item[(2)] $\Delta = (-2)^2 - 4 \times 1 \times 3 = 4 - 12 = -8 < 0$，\textbf{无实数根}
            \item[(3)] $\Delta = (-4)^2 - 4 \times 1 \times 4 = 16 - 16 = 0$，\textbf{两个相等实根}
        \end{enumerate}
        ...
        \begin{itemize}
            \item 首先，一元二次方程要求 $a \neq 0$，所以 $k \neq 0$
            \item 其次，有两个不等实根要求 $\Delta > 0$
            \item $\Delta = (-2)^2 - 4 \times k \times (-1) = 4 + 4k > 0$
            \item 解得 $4k > -4$，即 $k > -1$
            \item 综合：$k > -1$ 且 $k \neq 0$
        \end{itemize}
        
        \item \textbf{答案：$k = -\dfrac{1}{4}$} \\
        解析：有两个相等实根 $\Rightarrow \Delta = 0$
        \begin{align*}
...
\begin{answerbox}
    \begin{enumerate}
        ...
        由韦达定理：$x_1 + x_2 = \dfrac{5}{2}$，$x_1 x_2 = \dfrac{1}{2}$
        \begin{enumerate}
            \item[(1)] $x_1 + x_2 = \dfrac{5}{2}$，$x_1 x_2 = \dfrac{1}{2}$
            
            \item[(2)] $\dfrac{1}{x_1} + \dfrac{1}{x_2} = \dfrac{x_1 + x_2}{x_1 x_2} = \dfrac{\frac{5}{2}}{\frac{1}{2}} = 5$
            
            \item[(3)] $x_1^2 + x_2^2 = (x_1 + x_2)^2 - 2x_1 x_2 = \left(\dfrac{5}{2}\right)^2 - 2 \times \dfrac{1}{2} = \dfrac{25}{4} - 1 = \dfrac{21}{4}$
        \end{enumerate}
        
        \item \textbf{答案：另一个根是 2，$m = 2$} \\
        解析：设另一根为 $x_2$。
        \begin{itemize}
            \item 由两根之和：$1 + x_2 = 3$，得 $x_2 = 2$
            \item 由两根之积：$1 \times 2 = m$，得 $m = 2$
        \end{itemize}
        
        \item \textbf{答案：$y^2 - 6y + 7 = 0$} \\
        解析：设新方程的两根为 $y_1 = x_1 + 1$，$y_2 = x_2 + 1$。
        
        先求原方程：$x_1 + x_2 = 4$，$x_1 x_2 = 2$
        \begin{align*}
            y_1 + y_2 &= (x_1 + 1) + (x_2 + 1) = x_1 + x_2 + 2 = 4 + 2 = 6 \\
            y_1 \cdot y_2 &= (x_1 + 1)(x_2 + 1) = x_1 x_2 + x_1 + x_2 + 1 = 2 + 4 + 1 = 7
        \end{align*}
        新方程为 $y^2 - 6y + 7 = 0$
    \end{enumerate}
\end{answerbox}
...
\begin{answerbox}
    \begin{enumerate}
        ...
        化简：$10[1 + (1+x) + (1+x)^2] = 33.1$
        
        设 $t = 1+x$，则 $1 + t + t^2 = 3.31$
        ...
        \item \textbf{答案：(1) $y = -30x + 960$；(2) 定价 24 元，最大利润 1920 元} \\
        解析：
        \begin{enumerate}
            \item[(1)] 设 $y = kx + b$，代入 $(20, 360)$ 和 $(25, 210)$：
            \[\begin{cases} 20k + b = 360 \\ 25k + b = 210 \end{cases}\]
            解得 $k = -30$，$b = 960$，所以 $y = -30x + 960$
            ...
            \begin{align*}
                ...
                &= -30(x - 24)^2 + 1920
            \end{align*}
            当 $x = 24$ 时，$W_{\text{最大}} = 1920$ 元
        \end{enumerate}
        
        \item \textbf{答案：$AB = 20$ 米，$BC = 20$ 米} \\
        解析：设 $AB = x$ 米，则 $BC = (100 - 4x)$ 米（因为有 4 条竖着的围栏）
        ...
        \textbf{检验墙长限制}（墙长 25 米）：
        \begin{itemize}
            \item 当 $x = 5$ 时，$BC = 100 - 20 = 80 > 25$，超出墙长，舍去
            \item 当 $x = 20$ 时，$BC = 100 - 80 = 20 < 25$，符合
        \end{itemize}
        所以 $AB = 20$ 米，$BC = 20$ 米
    \end{enumerate}
\end{answerbox}
